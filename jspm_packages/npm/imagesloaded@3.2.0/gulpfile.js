...
(function(Buffer, process) {
  ...
  gulp.task('hint-js', function() {
    return gulp.src('imagesloaded.js').pipe(jshint()).pipe(jshint.reporter('default'));
  });
  ...
  gulp.task('requirejs', function() {
    ...
    return gulp.src('imagesloaded.js').pipe(rjsOptimize({
      baseUrl: 'bower_components',
      optimize: 'none',
      include: ['../imagesloaded']
    })).pipe(replace("'../imagesloaded',", '')).pipe(addBanner(banner)).pipe(rename('imagesloaded.pkgd.js')).pipe(gulp.dest('.'));
  });
  var uglify = require('gulp-uglify');
  gulp.task('uglify', ['requirejs'], function() {
    var banner = getBanner();
    gulp.src('imagesloaded.pkgd.js').pipe(uglify()).pipe(addBanner(banner)).pipe(rename('imagesloaded.pkgd.min.js')).pipe(gulp.dest('.'));
  });
  ...
  gulp.task('version', function() {
    ...
    gulp.src('imagesloaded.js').pipe(replace(/imagesLoaded v\d\.\d\.\d/, 'imagesLoaded v' + version)).pipe(gulp.dest('.'));
    gulp.src(['bower.json', 'package.json']).pipe(replace(/"version": "\d\.\d\.\d"/, '"version": "' + version + '"')).pipe(gulp.dest('.'));
    gulp.src('README.md').pipe(replace(/ajax\/libs\/jquery.imagesloaded\/\d\.\d\.\d/g, 'ajax/libs/jquery.imagesloaded/' + version)).pipe(gulp.dest('.'));
  });
